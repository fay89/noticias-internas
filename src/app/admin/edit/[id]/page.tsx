...
export default function EditArticlePage({ params }: { params: { id: string } }) {
  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (audio) {
        const audioStorageRef = ref(storage, `audios/${Date.now()}_${audio.name}`);
        const audioUploadTask = await uploadBytesResumable(audioStorageRef, audio);
        finalAudioUrl = await getDownloadURL(audioUploadTask.ref);
      }
      ...
      <form onSubmit={handlePublish} className={styles.form}>
        ...
        <div className={styles.grid2}>
          ...
          {category === 'Editorial' && (
            <>
              <div className={styles.formGroup}>
                <label>Firma de la Editorial</label>
                <input 
                  type="text" 
                  value={authorName} 
                  onChange={(e) => setAuthorName(e.target.value)} 
                  placeholder="Ej: El Equipo de Embajadores"
                  className={styles.inputTitle}
                />
              </div>
              <div className={styles.formGroup}>
                <label>Audio de Fondo (Opcional, dejar en blanco para mantener)</label>
                {currentAudioUrl && (
                  <div style={{ marginBottom: '10px', fontSize: '0.85rem', color: '#64748b' }}>
                    Audio actual subido. Selecciona otro si deseas reemplazarlo.
                  </div>
                )}
